...
import {setTestDetailsList} from '../../../redux/tabs/tabsAction';
...
function TestDetails(props) {
  ...
  const examDetailList = useSelector(state => state.tabs.testDetailsList);
  const userExamItem = useSelector(state => state.tabs.userExamItem);
  const dashStat = useSelector(state => state.tabs.dashStat);
    ...
    <>
      <Screen>
        <Header
          ...
          title={
            userExamItem.SubjectName ? userExamItem.SubjectName : 'Subject'
          }
          showBackButton={true}
          showDrawerBtn={false}
          onBackPress={() => {
