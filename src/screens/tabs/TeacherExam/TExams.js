...
import {setTeacherExams} from '../../../redux/tabs/tabsAction';
...
function TExams(props) {
  ...
  const filterExamItems = (text, from, to) => {
    if (text || from || to) {
      const newData = data.filter(item => {
        ...
        const timeStamp = moment(item.Date).format('yyyy-MM-DD');
  ...
  const getTests = () => {
    ...
    postApiCall(END_POINT.T_EXAMS, data)
      .then(res => {
        if (res.Code === 2) {
          ...
          if (Object.keys(res.Data).length == 0) {
            ...
            setFilteredData(res.Data.Exams);
            dispatch(setTeacherExams(res.Data));
          }
        } else if (res.Code === 0) {
          Commons.snackBar(res.Message, COLORS.primary);
        }
      ...
      <Screen>
        ...
        {user.AddExams == 1 && (
          <FloatingAction
            ...
            onPressItem={name => {
              if (name === 'bt_create') {
                Commons.navigate(props.navigation, ROUTES.T_EXAM_CREATE);
              }
            }}
          />
        )}
